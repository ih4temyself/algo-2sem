    ...
    for list_gen_func, size_tests_dict in custom_list_dict.items():
        ...
        for size, tests in size_tests_dict.items():
            ...
            for _ in range(tests):
                time_taken = sort_time_counter(
                    sorting_function, list_gen_func(size), size
                )
...
if __name__ == "__main__":
    ...
    add_logs.add_data_to_log(
        experimenting(InsertionSort.insertion_sort, custom_lists), "insertion_sort"
    )
    add_logs.add_data_to_log(
        experimenting(SelectionSort.selection_sort, custom_lists), "selection_sort"
    )
    add_logs.add_data_to_log(
        experimenting(ShellSort.shell_sort, custom_lists), "shell_sort"
    )
